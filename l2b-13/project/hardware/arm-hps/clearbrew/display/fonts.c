...
/*
	...
	This code was taken from ShreyansK2000/Translaite, from the following file:
		https://github.com/ShreyansK2000/Translaite/blob/master/Display/Fonts.c
	...
	switch(size) {
		case SMALL:
			for (x = 0; x < wordsize; x++) {
				print_character_small_font(x*(FONT1_XPIXELS+2) + x_offset, y_offset, colour, backgroundColour, word[x], 0);
			}
			break;
		case MEDIUM:
			for (x = 0; x < wordsize; x++) {
				print_character_medium_font(x*(FONT2_XPIXELS+2) + x_offset, y_offset, colour, backgroundColour, word[x], 0);
			}
			break;
		case LARGE:
			for (x = 0; x < wordsize; x++) {
				print_character_large_font(x*(FONT3_XPIXELS+2) + x_offset, y_offset, colour, backgroundColour, word[x], 0);
			}
			break;
		case GIANT:
			for (x = 0; x < wordsize; x++) {
				print_character_giant_font(x*(FONT4_XPIXELS+2) + x_offset, y_offset, colour, backgroundColour, word[x], 0);
			}
			break;
		case MASSIVE:
			for (x = 0; x < wordsize; x++) {
				print_character_massive_font(x*(FONT5_XPIXELS+2) + x_offset, y_offset, colour, backgroundColour, word[x], 0);
			}
			break;
	}
}
